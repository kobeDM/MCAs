...
{
  //printf("Is Aquiring Data function.\n");
    if (!IsInitialised())
        return ERROR_NOT_INITIALISED;
...
int DriverMgr::BeginDataAcquisitionF(unsigned int deviceID, unsigned int realTime, unsigned int liveTime, int64_t *ts_out,double A0, double A1, double A2,char fileName[])
{
  //printf("test DriverMgr arguments %s \n",fileName);
    if (!IsInitialised())
        return ERROR_NOT_INITIALISED;
	...
	{
	  int64_t ts=0;
	  int res = itDevice->second->BeginDataAcquisitionF(realTime, liveTime, &ts,A0,A1,A2,fileName) ? ERROR_OK : ERROR_UNKNOWN;
	  //printf("test DriverMgr arguments %ld \n",ts);
	  *ts_out=ts;
	  return res;
	}
	else
